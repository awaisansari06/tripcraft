...
export const Timeline = ({ data, scrollContainer, tripData }: { data: TimelineEntry[], scrollContainer?: React.RefObject<HTMLDivElement | null>, tripData: TripInfo }) => {
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-[40%]">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-3xl font-bold text-neutral-800 dark:text-neutral-100">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-neutral-800 dark:text-neutral-100">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
